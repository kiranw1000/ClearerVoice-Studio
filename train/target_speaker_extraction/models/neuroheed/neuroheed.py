...
class rnn(nn.Module):
    def __init__(self, args, N, B, H, K, R):
        ...
        encoder_layers = TransformerEncoderLayer(d_model=args.network_audio.d_model, nhead=1, dim_feedforward=args.network_audio.d_model*4)
...
def overlap_and_add(signal, frame_step):
    ...
    result = result.to(signal.device)  # Ensure result is on the same device as signal
